...
class MaterialService:
            ...
            for product in products:
                try:
                    ...
                    if material_id_str in materials:
                        using_products.append(product.name)
                except (json.JSONDecodeError, ValueError):
                    continue
            
            if using_products:
                if len(using_products) > 3:
                    product_text = f'{", ".join(using_products[:3])}等{len(using_products)}个产品'
                else:
                    product_text = ", ".join(using_products)
                return {'success': False, 'message': f'被{product_text}使用，请先删除相关产品'}
    ...
    def import_from_excel(self, file, username: str = '') -> dict:
        ...
        try:
            ...
            for row_num, row in enumerate(ws.iter_rows(min_row=2, values_only=True), start=2):
                # 新格式: 图片 | 材料名称 | 进价 | 售价 | 库存数量 | 创建时间 | 更新时间
                # 旧格式: 材料名称 | 进价 | 售价 | 库存数量
                
                self.logger.info(f'第{row_num}行: 列数={len(row)}, 数据={row}')
                
                # 判断是新格式还是旧格式
                # 新格式: 列数>=7 且 第一列不是有效的材料名称（空/图片路径/Excel错误值）
                first_col = str(row[0]) if row[0] else ''
                is_new_format = len(row) >= 7 and (not row[0] or first_col.startswith(('http', 'images/', '#')))
                
                if is_new_format:
                    name_idx, price_in_idx, price_out_idx = 1, 2, 3
                    self.logger.info(f'第{row_num}行: 使用新格式')
                else:
                    name_idx, price_in_idx, price_out_idx = 0, 1, 2
                    self.logger.info(f'第{row_num}行: 使用旧格式')
                
                if not row[name_idx]:  # 如果名称为空，跳过
                    self.logger.info(f'第{row_num}行: 名称为空，跳过')
                    continue
                ...
                try:
                    ...
                    self.logger.info(f'第{row_num}行: 名称={name}, 进价={in_price}, 售价={out_price}')
                    ...
                    if existing:
                        # 更新现有材料（保持现有图片）
                        self.logger.info(f'第{row_num}行: 材料已存在，更新 {name}')
                        self.update_material(existing.id, name, in_price, out_price, username, existing.image_path)
                        success_count += 1
                    else:
                        # 添加新材料
                        self.logger.info(f'第{row_num}行: 添加新材料 {name}')
                        result = self.add_material(name, in_price, out_price, username)
                        if result['success']:
                            success_count += 1
                            self.logger.info(f'第{row_num}行: 添加成功')
                        else:
                            failed_count += 1
                            self.logger.error(f'第{row_num}行: 添加失败')
                except Exception as e:
    ...
    def export_to_excel(self, material_ids: list = None):
        ...
        Args:
            material_ids: 要导出的材料ID列表，如果为空则导出所有材料
        """
        import openpyxl
