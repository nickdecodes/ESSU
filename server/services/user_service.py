...
class UserService:
    ...
    def _cleanup_expired_sessions(self, user):
        ...
        try:
            ...
            self.logger.warning(f'清理过期会话失败: {str(e)}')
            user.sessions = '{}'
    ...
    def logout(self, username: str, session_id: str = None) -> dict:
        """用户登出"""
        try:
            with self.db.session_scope() as session:
    ...
    def add_user(self, username: str, password: str, role: str, avatar_path: str = None) -> dict:
        """添加用户"""
        try:
            with self.db.session_scope() as session:
                user = User(
    ...
    def update_user(self, user_id: int, username: str = None, password: str = None, role: str = None, avatar_path: str = None) -> dict:
        """更新用户信息"""
        try:
            with self.db.session_scope() as session:
                ...
                if username and username != user.username:
                    existing = session.query(User.id).filter(User.username == username, User.id != user_id).first()
                    if existing:
                        return {'success': False, 'message': '用户名已存在'}
                ...
                self.logger.info(f'用户更新成功: ID={user_id}')
    ...
    def delete_user(self, username: str) -> dict:
        ...
        try:
            with self.db.session_scope() as session:
                user = session.query(User).filter(User.username == username).first()
                if not user:
                    self.logger.warning(f'删除用户失败: 用户不存在 - {username}')
                    return {'success': False, 'message': '用户不存在'}
    ...
    def get_all_users(self) -> dict:
        ...
        try:
            ...
            self.logger.error(f'获取用户列表异常: {str(e)}', exc_info=True)
            return {'success': False, 'message': '获取失败', 'users': []}
    
    def remove_user_session(self, username: str, session_id: str) -> dict:
        """移除用户指定会话"""
        try:
            with self.db.session_scope() as session:
                user = session.query(User).filter(User.username == username).first()
                    ...
                    if user.avatar_path:
                        try:
                            if user.avatar_path.startswith(('http://', 'https://')):
                                ...
                                avatar_file = os.path.abspath(avatar_file)
                                if os.path.exists(avatar_file) and avatar_file.startswith(os.path.abspath(Config.UPLOAD_FOLDER)):
